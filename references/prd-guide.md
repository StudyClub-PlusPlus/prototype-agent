...
- **없는 기능의 나열 (Hard Rule)** — 「탈퇴 사유를 묻지 않는다」처럼 **하지 않는 일**을 적지 않는다.
  기획서는 만들 것을 적는 문서다. 안 만드는 것을 적으면 읽는 사람은 그게 왜 거기 있는지 되묻는다.
  - 「~하지 않는다」는 **그 자리에 그것을 기대할 이유가 있을 때만** 쓴다 — 관행이라 넣을 뻔했거나,
    앞선 결정과 반대라 헷갈릴 때. 그때도 **왜 안 하는지**를 함께 적는다
    (예: *「예약 공개는 두지 않는다 — 준비가 됐는지는 날짜가 판단할 수 없다」*)
