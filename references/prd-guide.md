...
### 1.0-B 기획 찌꺼기 금지 (Hard Rule · 출력 직전 전수 스캔)

**찌꺼기 = 결정이 바뀌면서 쓸모가 끝났는데 문장으로 남은 것.** 접은 규칙, 탈락한 안, 옛 기준과의 비교,
「왜 A가 아니라 B인가」. 읽는 사람은 이게 아직 살아 있는 논점인 줄 알고 다시 논의를 연다.

**기획자는 결정으로 말한다.** 막기로 했으면 막는 규칙을 쓰고, 안 막기로 했으면 **그 줄을 지운다.**
「막지 않는다」로 고쳐 쓰는 것은 결정이 아니라 결정의 잔해다.

**출력 직전, 아래 표현을 본문 전체에서 찾아 전수 처리한다.** 하나라도 남으면 출력하지 않는다.

| 찾을 표현 | 처리 |
|---|---|
| 막지 않는다 · 거부하지 않는다 · 차단하지 않는다 | 지운다. 실제로 일어나는 일을 긍정형 한 줄로 (「요청을 그대로 처리한다」) |
| 묻지 않는다 · 받지 않는다 · 두지 않는다 · 넣지 않는다 | 지운다. 화면에 없다는 사실은 명세가 이미 말한다 |
| 더 이상 · 이제는 · 기존에는 · 이전에는 · 원래는 | 지운다. 문서는 지금 만들 것만 말한다 |
| ~였지만 · ~하려 했으나 · 검토했지만 · 초안에서는 · 원안은 | 지운다. 탈락한 안은 문서에 자리가 없다 |
| ~로 바꿨다 · ~에서 ~로 · 대신 ~한다 | 바꾼 뒤의 상태만 남긴다 |
| 없다 · 없음 (완료 기준 안에서) | 지운다. 확인할 동작이 없는 줄은 완료 기준이 될 수 없다 |

**치환 공식**: 지운다 → 그 자리에서 **실제로 일어나는 일**이 있으면 긍정형 한 줄로 쓴다 → 없으면 그냥 빈다.

```
(X) 네비게이터도 마지막 캡틴도 탈퇴를 막지 않는다
(X) 거부 조건을 두지 않는다 — 예전엔 409 로 막으려 했으나 접었다
(O) 모든 탈퇴 요청을 그대로 처리한다 — 요청자가 누구인지 따지지 않는다
```

**예외는 둘뿐이다.**
1. **개발자가 관행대로 넣을 위험이 실재할 때** — 비고에 **이유 없이 한 줄** (「자동 갱신 없음」)
2. **이미 배포된 계약을 뒤집을 때** — 시스템 요건에 「기존 계약 변경」으로 한 줄. 무엇이 어떻게 바뀌는지만

이 규칙은 **완료 기준·동작·정책·데이터·처리·비고 전부**에 적용된다. §1.4-C(비고)와 §3(없는 기능의
나열)은 이 규칙을 각 자리에서 다시 말한 것이며, 충돌하면 이 절이 이긴다.
...
· **완료 기준** — **이 Story 가 하는 일은 여기서만 말한다.** 별도의 「하는 일」 절을 두지 않는다 —
  ...
  - **막는 기준이 있을 때만 막는 것을 쓴다.** 「막는 조건이 없다」·「거부하지 않는다」는 완료 기준이 아니다 —
    확인할 동작이 없어 통과·실패를 가릴 수 없다. 막으려다 접은 안이 이 모양으로 남는다.
    막지 않기로 했으면 **막는 줄을 지우면 끝**이고, 요청이 그대로 처리된다는 사실은 §3 처리에 한 줄로 적는다
...
**어떤 화면을 캡처할 것인가 (Hard Rule)**

**그림을 봐야 알 수 있는 화면만 찍는다.** 캡처는 읽는 사람의 시간을 쓰는 자리이므로,
글 한 줄로 끝나는 것에 그림을 붙이면 정작 봐야 할 그림이 묻힌다.

- 찍는다 — 배치·격자·표처럼 **글로 옮기면 길어지는 것**, 값이 여럿 얽힌 화면(권한표·명단·대시보드),
  조건부로만 보이는 요소에 번호를 단 경우
- 찍지 않는다 — **카피 한 줄이 전부인 상태**(빈 결과·에러·로딩). 「상태별 화면」 표의 카피 칸이
  이미 그 화면을 다 말한다
- 같은 표·같은 값을 **두 문서에 그림으로 박지 않는다.** 한쪽만 낡는다 — 소관 Story 에 한 번 넣고
  다른 문서는 링크로 가리킨다
...
### 1.4-C 지나간 기획을 남기지 않는다 (Hard Rule · §1.0-B 의 비고 적용)
...
| 마지막 캡틴도 탈퇴를 막지 않는다 | 막기로 했다가 접은 규칙의 찌꺼기 — 막는 줄을 지우면 그만이다 |

비고는 **정해진 것**만 적는다. 아직 안 정해진 것은 문서에 넣지 않는다 (§3).
...
| ID | 정규문장 | 상태 | 본문 |
|---|---|---|---|
| ST-012 | 운영자는 스터디를 등록할 수 있다. | 기획완료 | [PRD](../../stories/study-create/PRD.md) |
...
- **미확정·오픈 이슈 목록 (Hard Rule)** — 「미확정」·「정해야 할 것」·「논의 필요」를 문서에 남기지 않는다.
  **기획서는 정해진 것을 적는 문서다.** 안 정해진 것을 적어 두면 읽는 사람은 무엇을 만들지 알 수 없고,
  정하는 일을 디자이너·개발자가 떠안게 된다.
  - 쓰다가 결정이 필요해지면 **문서에 적지 말고 기획자에게 묻는다.** 답을 받아 본문에 반영한다
  - 정말 답이 없으면 그 항목을 **범위 밖으로 빼고** 비고에 한 줄로 적는다 (「이번 범위에 없음」)
  - 구현이 아직 안 된 사실은 미확정이 아니다 — 비고의 **미구현**으로 적는다
  - 백엔드에 넘길 값(필드·enum·API)은 미확정이 아니라 **요청**이다. 시스템 요건에 정한 값으로 적는다

- **없는 기능의 나열 (Hard Rule · §1.0-B)** — 「탈퇴 사유를 묻지 않는다」처럼 **하지 않는 일**을 적지 않는다.
  기획서는 만들 것을 적는 문서다. 안 만드는 것을 적으면 읽는 사람은 그게 왜 거기 있는지 되묻는다.
  - 「~하지 않는다」를 쓸 수 있는 경우는 **§1.0-B 의 예외 둘뿐**이다 — 개발자가 관행대로 넣을 위험이
    실재할 때(비고에 **이유 없이 한 줄**: 「예약 공개 없음」), 이미 배포된 계약을 뒤집을 때(시스템 요건에 한 줄)
  - 그냥 범위에 없는 것은 비고의 **범위 밖** 한 줄로 끝낸다
  - **접은 규칙은 문장을 고치는 게 아니라 지운다** (§1.0-B 치환 공식)
    (「네비게이터면 409 로 거부한다」 → 「거부 조건을 두지 않는다」(X) → 「모든 탈퇴 요청을 그대로 처리한다」(O))

- **문서 첫머리의 소관 안내 (Hard Rule)** — 본문 시작부터 「~는 다른 Story 소관」을 적지 않는다.
  읽는 사람은 이 문서가 무엇을 다루는지 알러 온 것인데, 첫 줄이 **다루지 않는 것**을 말하면 헷갈린다.
  - 범위 경계가 필요하면 **비고의 「범위 밖」** 한 줄로 끝낸다 — 문서 끝이 그 자리다
  - 이어지는 다른 Story 로 보내야 할 때만 링크를 건다. 그때도 본문 첫머리가 아니라 그 값을 다루는 번호 안에서
...
- [ ] **§1.0-B 찌꺼기 표현을 본문 전체에서 전수 스캔했는가** — 막지 않는다 · 거부하지 않는다 ·
      묻지 않는다 · 두지 않는다 · 더 이상 · 기존에는 · ~였지만 · 검토했지만 · ~로 바꿨다 ·
      (완료 기준 안의) 없다. **0건**이어야 출력한다. 예외 둘(관행 방지 한 줄 · 기존 계약 변경)만 남긴다
...
- [ ] **카피 한 줄이 전부인 상태(빈 결과·에러·로딩)를 캡처로 넣지 않았는가** — 표의 카피 칸으로 충분하다
- [ ] **다른 문서에 이미 그림으로 있는 표를 또 박지 않았는가** (링크로 가리킨다)
...
- [ ] **「미확정」·「오픈 이슈」 목록이 문서에 남아 있지 않은가** — 남았으면 기획자에게 묻고 본문에 반영한다
- [ ] **본문 첫머리에 「~는 다른 Story 소관」류 안내가 없는가** (범위 경계는 비고에서)
- [ ] **하지 않는 일을 굳이 적지 않았는가** — 기대할 이유가 있을 때만, 왜 안 하는지와 함께
